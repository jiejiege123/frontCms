  ...
  methods: {
    ...
    submitForm(formName, status) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          const method = this.dialogType === 'add' ? addCategories : updateCategories
    ...
    getCategoriesAllData() {
      ...
      getCategoriesAll().then(res => {
        ...
        if (this.ruleForm.id) {
          ...
          const index = tableData.findIndex(n => n.id === this.ruleForm.id)
          tableData.splice(index, 1)
          this.$set(this.dics, 'pid', tableData)
          console.log(this.dics.pid)
        } else {
          this.$set(this.dics, 'pid', tableData)
        }
      }).catch(err => {
