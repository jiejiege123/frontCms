 ...
 * @Author: your name
 * @Date: 2019-09-05 20:04:03
 * @LastEditTime: 2019-09-06 11:28:25
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <span v-if="verifyFlag" class="verify-msg" v-text="finishtip" />
        <span v-else class="verify-msg" v-text="finishText" />
        <div
...
export default {
  ...
  computed: {
    ...
    ...mapGetters(['finishtip', 'verifyFlag', 'loading'])
  },
  watch: {
    // type变化则全面刷新
    type: {
        ...
        if (this.type !== '1') {		// 图片滑动
          ...
          if (parseInt((this.moveBlockLeft || '').replace('px', '')) >= (parseInt(this.setSize.barWidth) - parseInt(this.barSize.height) - parseInt(this.vOffset))) {
            ...
            this.$parent.$emit('success', (res) => {
              this.finishText = this.finishtip
            })
          } else {
            this.finishText = ''
            this.moveBlockBackgroundColor = '#d9534f'
